...
def inverse(target_x, target_y, target_z):
    ...
    i=0
    while(i<4):
        q=np.array([0+i*random.uniform(-np.pi/2, np.pi/2),0+i*random.uniform(-np.pi/2, np.pi/2),0+i*random.uniform(-np.pi/2, np.pi/2),0+i*random.uniform(-np.pi/2, np.pi/2),0+i*random.uniform(-np.pi/2, np.pi/2)], dtype=float)
        start=time.time()
        while(error>1):
